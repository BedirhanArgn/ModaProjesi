...
window.app = {
  ...
  joinConversation: function (convId) {
    //var convId = $('#convId').val();
    nextRTC.join(convId);
  },
  leaveConversation: function (convId) {
    $('.remotestream').remove();
    nextRTC.leave();
  },
  upperCase: function () {
...
var IsObjNew = function (children, dest){
	for(var i=0; i<children.length;i++){
		if(children[i].id == dest[0].id){
			return false;
		}
	}
	return true;
...
nextRTC.on('localStream', function (stream) {
  ...
  if($('#container')[0].children.length==0 || IsObjNew($('#container')[0].children, dest)){
	  	$("#container").append(dest);
	  	dest[0].style.display = "none";
  }
